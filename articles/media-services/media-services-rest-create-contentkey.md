<properties 
    pageTitle="Stvaranje ContentKeys s OSTATKOM | Microsoft Azure" 
...
#<a name="create-contentkeys-with-rest"></a>Stvaranje ContentKeys s OSTATKOM
...
Media Services omogućuje vam da biste stvorili novi i izlaganje šifrirane resursi. **ContentKey** omogućuje siguran pristup vaše **resursa**s. 

Kada stvorite novu imovinu (na primjer, prije [prijenosa datoteka](media-services-rest-upload-files.md)), možete odrediti sljedeće mogućnosti šifriranja: **StorageEncrypted**, **CommonEncryptionProtected**ili **EnvelopeEncryptionProtected**. 

Kada isporuka imovine klijentima, možete [konfigurirati za imovinu dinamički šifriranje](media-services-rest-configure-asset-delivery-policy.md) s jednim od sljedeće dvije encryptions: **DynamicEnvelopeEncryption** ili **DynamicCommonEncryption**.
...
1. Nasumično generirati ključa AES 16-bajtni (za šifriranje zajednički i omotnica) ili ključa AES 32-bajtni (za šifriranje prostora za pohranu). 
...
3.  Šifriranje ključa sadržaja s javnim ključem certifikat X.509. 
    ...
    Media Services .NET SDK koristi RSA sa OAEP pri izvođenju šifriranje.  Vidjet ćete primjer u [EncryptSymmetricKeyData (opis funkcije)](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4.  Stvorite kontrolni zbroj vrijednost (koja se temelji na algoritam ključa kontrolni zbroj PlayReady AES) izračunava se pomoću ključne identifikator i sadržaja ključ. Dodatne informacije potražite u odjeljku "PlayReady AES kontrolni zbroj algoritam ključa" PlayReady zaglavlja objekt dokumenta nalazi [ovdje](http://www.microsoft.com/playready/documents/).

    U sljedećem primjeru .NET izračunava kontrolni zbroj pomoću GUID dio identifikator ključa i tipku Očisti sadržaj.
...
6. Entitet **ContentKey** pridružiti vaše entitet **resursa** kroz postupak $links.

Imajte na umu primjeri koji generiranje ključa AES šifriranje tipku i izračun u kontrolni zbroj ste je izostavljena u ovoj se temi. Primjeri koji pokazuju kako raditi s Media Services postoje.
...
##<a name="create-the-contentkey"></a>Stvaranje na ContentKey 
...
Jedna od vrijednosti koje morate postaviti kada stvorite sadržaj ključ je vrsta. Odaberite neku od sljedećih vrijednosti.
