<properties 
    pageTitle="Implementacijom prebacivanje strujanje scenarij | Microsoft Azure" 
    description="U ovoj se temi objašnjava implementirati prebacivanje strujanje scenarij." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementacijom prebacivanje strujanje scenarija

Ovaj vodič opisuje način da biste kopirali sadržaj (BLOB polja) s jednom resursa u drugu da bi se rukovati zalihosti za strujanje na zahtjev. Ovaj scenarij je korisna korisnicima koji želite postaviti svoje CDN za prebacivanje između dva podatkovnim centrima slučaju do prekida u jednom od naše centre za podatke.
Ovaj vodič koristi Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST API-JA i SDK za pohranu Azure da bismo pokazali sljedeće zadatke.

1. Postavljanje računa Media Services u "Data Center A".
1. Prijenos datoteke mezzanine u izvoru sredstava.
1. Kodiranje imovine u većim brojem bit stopa MP4 datoteke. 
1. Stvaranje samo za čitanje SAS locator imovine izvora da biste imali pristup čitanju spremnik na računu za pohranu koji je pridružen resursa izvora.
1. Dohvaćanje naziva spremnik resursa izvora iz samo za čitanje SAS Lokator stvorili u prethodnom koraku. Ne možemo vam informacije biti potrebne za kopiranje blob-ova između računa za pohranu (pojedinosti objašnjene u temi.)
1. Stvaranje programa locator polazište za imovinu koja je stvorena pomoću kodiranja zadatka. 

Zatim, učiniti na prebacivanje:

1. Postavljanje računa Media Services u "Podataka centra B".
1. Stvaranje ciljne resursa za prazan u ciljnoj računa za servise za medijske sadržaje.
1. Stvaranje locator za SAS pisanje za cilj prazan resursa da biste imali pristup pisanje spremnik u ciljnoj račun za pohranu koji je pridružen resursa cilj.
1. Pomoću SDK za pohranu Azure kopirajte blob-ova (resursa datoteke) između izvora račun za pohranu u "Data Center A" i ciljne prostora za pohranu računa u "Podataka centra B" (te račune za pohranu pridružuju resursi koji vas zanimaju.)
1. Pridruživanje blob-ova (resursa datoteke) kopiranih u spremniku blob cilj s ciljnom resursa. 
1. Stvaranje programa locator polazište za resursa u "Podataka centra B" i navedite locator Id koja je stvorena za resursa u "Data Center A". 
1. Tako ćete dobiti strujanje URL-ovi gdje relativni putovi URL-ove jednaki (samo na bazu razlikuju URL-ovi). 
 
Nakon toga učiniti sve kvarove možete stvoriti CDN pri vrhu ove locators polazište. 

Primjena Imajte na umu sljedeće:

- Trenutnu verziju Media Services SDK ne podržava stvaranje na locator navedeni locator ID-om. Da biste postigli ovaj zadatak, koristit ćemo Media Services REST API-JA.
- Trenutna verzija Media Services SDK ne podržava programski generiranje IAssetFile informacije koje sredstvo želite pridružiti datoteka resursa. Da biste postigli ovaj zadatak, koristit ćemo CreateFileInfos Media Services REST API-JA. 
- Resursi za pohranu šifrirane (AssetCreationOptions.StorageEncrypted) nisu podržana za replikaciju (jer ključa za šifriranje će biti različiti u oba računa Media Services). 
- Ako želite da iskoristite prednost dinamički pakiranje, najprije morate dobiti barem jedan osvježavati strujanje rezervirane jedinice. Dodatne informacije potražite u članku [Dinamički pakiranje resursi](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Razmislite o korištenju Media Services [Umnažanje alata](http://replicator.codeplex.com/) kao zamjena za ručno implementacijom prebacivanje strujanje scenarij. Ovaj alat omogućuje replicirati imovine preko dva računa Media Services.

##<a name="prerequisites"></a>Preduvjeti
 
- Dva računa Media Services u novu ili postojeću pretplatu Azure. Potražite u članku [Stvaranje računa za servise medijske sadržaje](media-services-portal-create-account.md).
- Operacijske sustave: Windows 7, Windows 2008 R2 ili Windows 8.
- .NET framework 4,5 ili .NET Framework 4.
- Visual Studio 2010 SP1 ili noviju verziju (Professional, Premium, Ultimate ili Express).
 
##<a name="set-up-your-project"></a>Postavljanje projektu

U ovom odjeljku će stvoriti i postavljanje aplikacije konzole za C# projekta.

1. Koristite Visual Studio za stvaranje novog rješenja koja sadrži aplikacije konzole za C# projekta. Unesite HandleRedundancyForOnDemandStreaming naziv, a zatim kliknite u redu.
1. Stvaranje mape SupportFiles na jednakoj razini kao datoteku HandleRedundancyForOnDemandStreaming.csproj projekta. U mapi SupportFiles stvorite mape OutputFiles i MP4Files. Kopiranje .mp4 datoteke u mapu MP4Files (u ovom primjeru se koristi datoteka BigBuckBunny.mp4). 
1. Koristite **Nuget** za dodavanje reference na povezane DLL bibliotekama Media Services. U Visual Studio glavni izbornik, odaberite Alati -> Upravitelj paketa biblioteke -> konzole za Upravitelj paketa. U prozor unesite konzole za instalaciju paketa windowsazure.mediaservices i pritisnite tipku enter.
1. Dodajte ostale reference koji su potrebni za taj projekt: System.Configuration, System.Runtime.Serialization i sadrži.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Dodajte kod koji rukuje zalihosti za strujanje na zahtjev.



1. Dodajte sljedeća polja predmete razinom predmete Program.
...
1. Zamijenite jedna definiciju zadane glavne metoda:
...
1. Način definicije koje se nazivaju iz glavnog definirane su u nastavku.
...
Sada možete koristiti upravitelj promet na zahtjeve za usmjeravanje između dva podataka centrima i stoga prebacivanje slučaju sve kvarove.
