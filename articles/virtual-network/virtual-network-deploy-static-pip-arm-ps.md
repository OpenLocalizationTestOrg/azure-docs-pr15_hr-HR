<properties 
   pageTitle="Implementacija VM pomoću statičke IP javno pomoću komponente PowerShell u upravitelju resursa | Microsoft Azure"
   description="Saznajte kako implementirati VMs pomoću statičke IP javno pomoću komponente PowerShell u upravitelju resursa"
...
# <a name="deploy-a-vm-with-a-static-public-ip-using-powershell"></a>Implementacija VM pomoću statičke IP javno pomoću komponente PowerShell
...
Možete preuzeti cijeli skriptu PowerShell koristi [ovdje](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/03-Static-public-IP/virtual-network-deploy-static-pip-arm-ps.ps1). Slijedite korake u nastavku da biste promijenili skripta za rad u svom okruženju.
...
4. Stvaranje mrežnog sučelja (NIC) za VM u podmreže iznad stvorenih javnu IP. Obratite pozornost na prvi cmdlet dohvaćanja na VNet iz Azure, to je potrebno jer **Skup AzureRmVirtualNetwork** izvršen da biste promijenili postojeću VNet.
...
1. Stvaranje objekta konfiguracije za na VM.

        $vmConfig = New-AzureRmVMConfig -VMName $vmName -VMSize $vmSize 

1. Dobiti vjerodajnice VM lokalnog administratorskog računa.

        $cred = Get-Credential -Message "Type the name and password for the local administrator account."

2. Stvaranje objekta VM konfiguracije.

        $vmConfig = Set-AzureRmVMOperatingSystem -VM $vmConfig -Windows -ComputerName $vmName `
            -Credential $cred -ProvisionVMAgent -EnableAutoUpdate

3. Postavljanje slike operacijski sustav za na VM.

        $vmConfig = Set-AzureRmVMSourceImage -VM $vmConfig -PublisherName $publisher `
            -Offer $offer -Skus $sku -Version $version

4. Konfiguriranje OS disk.

        $osVhdUri = $stdStorageAccount.PrimaryEndpoints.Blob.ToString() + "vhds/" + $osDiskName + ".vhd"
        $vmConfig = Set-AzureRmVMOSDisk -VM $vmConfig -Name $osDiskName -VhdUri $osVhdUri -CreateOption fromImage

5. NIC-a dodati na VM.
...
1. S konzole PowerShell ili Očisti filtar, pokrenuti skriptu iznad.
